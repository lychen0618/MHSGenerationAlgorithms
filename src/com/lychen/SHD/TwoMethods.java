...
//    }
    ...
    public void dfs(Hypergraph crit, BitSet uncov, Hypergraph H, Hypergraph T, BitSet S, BitSet cand) {
        ...
        for (int j = c.nextSetBit(1); j != -1; j = c.nextSetBit(j + 1)) {
            if (!shd.vertex_would_violate(crit, uncov, H, T, S, j)) {
                HashMap<Integer, BitSet> temp = shd.update_crit_and_uncov(crit, uncov, H, T, S, j);
                BitSet ss = (BitSet) S.clone();
                ss.flip(j);
