...
public class Shd {
    public boolean vertex_would_violate(Hypergraph crit, BitSet uncov, Hypergraph H, Hypergraph T, BitSet S, int v) {
        assert !S.get(v);
    ...
    public HashMap<Integer, BitSet> update_crit_and_uncov(Hypergraph crit, BitSet uncov, Hypergraph H, Hypergraph T, BitSet S, int v) {
        assert !S.get(v);
        assert crit.getEdges().get(v).isEmpty();
